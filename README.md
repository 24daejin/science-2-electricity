...
- **학생**: 학번 + 이름 + 반 공통 로그인코드(교사가 반코드 탭에서 관리, 매 수업 칠판 등으로 안내)
- **교사**: 이름 + 비밀번호(스크립트 속성에 저장)

로그인 성공 시 Apps Script가 임시 `authToken`(CacheService 기반, 6시간 유효)을 발급하고, 이후 모든 요청에 이 토큰을 함께 보냅니다. Google 계정이 신원을 보증해주지 않으므로, 같은 반 학생끼리는 서로 다른 학번으로 로그인할 수 있다는 한계가 있습니다 — 교실 내 활동 맥락에서 감수 가능하다고 판단해 채택했습니다. (추후 Cloud Console 접근이 열리면 [docs/OAUTH_SETUP.md](docs/OAUTH_SETUP.md)의 절차로 GIS 방식으로 전환할 수 있도록 문서를 남겨두었습니다.)

## 아키텍처

```
[학생/교사 브라우저]
   │  자체 로그인(학번+이름+반코드 / 교사 이름+비밀번호) → authToken 발급
   ▼
...
[Google Apps Script 웹앱: apps-script/]  doGet/doPost 라우팅
   │  ① authToken을 CacheService로 검증(역할: 학생/교사, 학번, 이름)
...
| 형성평가_문항 | 소단원ID(1~6)로 구분된 54문항 | ⏳ 병합 예정 (아직 미생성) |
| 학생명단 | 학번·이름·반·상태·비고(이메일은 기록용, 선택) | ⏳ 병합 예정 (아직 미생성) |
...
(형성평가_문항·학생명단 탭이 이 파일에 병합되면 별도 설정 변경 없이 자동으로 반영됩니다.)
...
- [x] 진단평가 웹앱
- [x] 형성평가 웹앱 (6개 소단원 공용) — 코드는 완성, 형성평가_문항 탭 데이터 병합 후 실제 문항으로 테스트 필요
- [x] 오답 재학습 → 챗봇 라우팅
- [x] 파인만 챗봇
- [x] 교사 주도 실시간 진행 제어 + 이탈 감지
- [x] 명단 관리 CRUD + 반코드 관리 — 코드는 완성, 학생명단 탭 데이터 병합 후 실제 명단으로 테스트 필요
- [x] 배포 가이드 문서

미완료: 학생명단·형성평가_문항 탭 병합, 교사 계정 비밀번호/반코드 실값 설정, 실배포 후 end-to-end 테스트.
