...
- [x] 학생 홈 화면: 선생님이 반에 지정한 소단원만 카드로 표시(지정 전엔 아무 카드도 안 보임), 등록된 자료는 "활동" 토글로 묶고, 클릭하면 새 탭 대신 팝업(닫기 버튼 포함)으로 열림
- [x] 버그 수정: 챗봇 응답이 max_tokens 부족으로 문장 중간에 잘리던 문제(300→600 상향) + 진행 카운터가 "봇 질문 횟수"가 아니라 "행 번호" 기준이라 실제보다 일찍 N/5로 보이던 문제 수정(실제 저장된 대화 로그로 확인)
...
- [x] 학생 홈 화면 재구성: "오늘의 활동" 드롭다운 — 소단원 선택 후 그날의 활동(진단평가 포함)을 목록으로 보여주되 교사가 활성화한 것만 클릭 가능, 나머지는 비활성 표시(반_활성활동 시트 신설, 세션_상태의 문항별 세부 제어와는 별개)
