...
- [x] 학생 본인용 "내 대시보드"(`my-dashboard/`) 신설 — 점수, 소단원별 형성평가 정답률 막대그래프, "진단평가→형성평가" 성장 비교 그래프, 승인된 "관련 내용 답하기" 평가(별점·근거)를 보여줌. 반 평균/친구 비교는 자존감 보호를 위해 일부러 뺐고 "나 vs 예전의 나" 성장에만 집중. 백엔드는 `Dashboard_getMyDashboard`(교사용 `Dashboard_getStudentDetail`과 달리 항상 `auth.seq` 본인만 조회)를 새로 추가하고, 소단원별 집계 로직은 `Dashboard_getClassSummary`와 `Dashboard_buildStudentSubunitSummary_`로 공유해 중복 제거
