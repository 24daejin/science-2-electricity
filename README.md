...
[Google Sheets: 마스터 스프레드시트]      [Firestore: diagnosticQuestions/
  ← 교사가 편집하는 SSOT                     classActiveActivity]
                                              ▲
                                              │ 브라우저가 직접 읽음(빠름, 폴링 없음)
                                    [학생/교사 브라우저]
```

- **왜 Sheets와 Firestore를 같이 쓰나**: 애초에 GitHub Pages + Apps Script(+ Google Sheets SSOT) 조합만 쓰기로 확정했었지만, Apps Script가 요청마다 스프레드시트를 새로 여는 구조라 느렸습니다(도입 당시엔 형성평가 화면의 2.5초 폴링이 특히 문제였는데, 형성평가는 이후 종이 학습지로 옮겨져 그 화면 자체가 없어졌습니다). 그래서 **교사의 편집 경험은 그대로 Sheets로 유지**하되, **학생이 자주 읽는 데이터(문항, 반별 활성 활동)만 Firestore로 미러링**해서 그 부분만 빠르게 만들었습니다 — 로그인·응답저장·챗봇·명단관리처럼 정확성이 중요하고 반복 호출되지 않는 부분은 그대로 Apps Script+Sheets입니다. 자세한 설정은 [docs/FIREBASE_SETUP.md](docs/FIREBASE_SETUP.md) 참고.
...
- **실시간 동기화**: 반별 활성 활동은 폴링하지 않고, 학생 브라우저가 Firestore `classActiveActivity` 문서를 실시간 리스너(`onSnapshot`)로 직접 구독합니다. 교사가 메인 화면에서 활동 체크박스를 켜면 즉시 Firestore에 반영되고, 그 순간 모든 학생 화면이 즉시 갱신됩니다.
...
| 형성평가_문항 | 소단원ID(1~6)로 구분된 54문항 | 형성평가는 종이 학습지로 진행 — 문항 자체는 안 쓰고, 챗봇 개념→성취기준 매핑용 참조로만 사용 |
| 학생명단 | 순번·학년·반·번호·이름·학적 | ✅ 운영 중 (262명) |
| 반코드 | 반별 로그인 공통 코드 | Apps Script가 최초 실행 시 자동 생성, roster/ 화면에서 관리 |
| 진단평가_응답 | 학생 응답, 확신도, 정오답, 제출시각 | Apps Script가 최초 실행 시 자동 생성 |
| 형성평가_응답 | (더 이상 안 씀) | 종이 학습지 전환 이전의 과거 기록만 남아있음 |
...
"Firestore로 동기화됨" 표시된 탭은 `diagnosticQuestions` Firestore 컬렉션으로 자동 미러링되고, `반_활성활동`은 `classActiveActivity` 컬렉션으로 저장 시점에 바로 반영되어, 학생 화면은 시트가 아니라 Firestore에서 직접 읽습니다(속도 때문 — [docs/FIREBASE_SETUP.md](docs/FIREBASE_SETUP.md) 참고). 교사는 여전히 시트(또는 메인 화면)에서만 편집합니다.
...
- [x] 진단평가 웹앱
- [x] 형성평가 웹앱 (6개 소단원 공용) — 실제 문항 데이터 반영, 헤더 행 수정 완료 (→ 이후 종이 학습지로 전환, 앱 화면 자체가 제거됨. 아래 "형성평가 앱 제거" 참고)
- [x] 오답 재학습 → 챗봇 라우팅
...
- [x] 형성평가 "일괄 배포" 모드(소단원 전체 문항을 학생이 자기 속도로 풀이) (→ 이후 유일한 모드가 됐다가, 형성평가 자체가 종이 학습지로 전환되며 제거됨)
- [x] 홈 화면 소단원별 재구성 + 챗봇 명칭을 "관련 내용 답하기"로 변경
- [x] 교사 대시보드(반별 → 학생별 정답률/진행률, 학생별 전체 이력 — 생기부 참고자료, CSV/텍스트 일괄 다운로드)
- [x] 점수 시스템(형성평가 정답 시 적립, 문제풀이/교과서활동 유형 구분) (→ 형성평가 제거 후 교과서활동 유형만 남음)
...
- [x] 대시보드: 소단원별로 형성평가 결과 + 챗봇 대화 결과(별점) 통합 표시, 소단원별 정답률 막대그래프, "전체 학급 한눈에 보기"(반별 비교 히트맵) (→ 형성평가 결과·정답률 막대그래프는 이후 제거, 챗봇 참여만 남음)
...
- [x] 대시보드: 문항별 정답 추이(재도전 시 성장), 반 성장 비교(진단평가 vs 형성평가 정답률), 이탈 로그 노출 (→ 형성평가 관련 추이/성장 비교는 이후 제거)
...
- [x] 반별 활동 동시 활성화 — 교사가 같은 소단원 안에서 여러 활동을 체크박스로 동시에 켤 수 있음(반_활성활동의 활성활동키를 콤마로 이어붙인 배열로 확장). 관련 내용 답하기는 배운 내용을 점검하는 활동이라 목록 맨 뒤(등록 자료 다음)로 순서 조정
...
- [x] 죽은 코드 정리 — 안 쓰는 백엔드 라우트(`getDiagnosticQuestions`/`getFormativeQuestions`, Firestore 마이그레이션 뒤로 호출하는 곳이 없었음), 아무 효과 없던 프론트 설정값(`CHATBOT_MAX_TURNS`, 실제 값은 백엔드 상수가 정함), 안 쓰는 CSS 3개(`.card-columns`/`.menu-grid`/`footer.disclaimer`) 제거. Firestore 연동이 "선택"이 아니라 사실상 필수라는 점을 문서에 명확히 반영
- [x] 반별 활동 지정 카드를 2열 그리드로 배치(반이 많아도 세로로 한없이 길어지지 않게)
- [x] 담당 반 권한 분리 — `TEACHER_ACCOUNTS`에 교사별 `classrooms` 배열을 지정하면(예: 1~8반/9~10반) 교사 화면들(반별 활동 지정·학생 진행 대시보드·학생명단/반코드 관리·챗봇 평가 검수)이 기본으로 담당 반만 보여줌. "전체 반 보기" 토글로 언제든 다른 반도 볼 수 있는 화면 정리용 분리이며, 서버 쪽에서 강제로 막는 건 아님. `classrooms`를 안 넣은 계정은 예전처럼 모든 반이 보임(하위 호환)
- [x] 교사 메뉴 순서 변경(학생명단·반코드 관리 → 대시보드 → 챗봇 평가 검수) + "학생 진행 대시보드"를 "대시보드"로 명칭 단순화, 챗봇 평가 검수 문구 "학부모 포털에 공개됩니다" → "학부모에게 공개 가능해집니다"로 수정
- [x] 헤더 badge에서 아이콘 이미지 제거, 그라디언트 사각형만 남김(모든 페이지 공유 CSS 한 곳만 수정)
- [x] 학생 본인용 "내 대시보드"(`my-dashboard/`) 신설 — 점수, 소단원별 형성평가 정답률 막대그래프, "진단평가→형성평가" 성장 비교 그래프, 승인된 "관련 내용 답하기" 평가(별점·근거)를 보여줌. 반 평균/친구 비교는 자존감 보호를 위해 일부러 뺐고 "나 vs 예전의 나" 성장에만 집중. 백엔드는 `Dashboard_getMyDashboard`(교사용 `Dashboard_getStudentDetail`과 달리 항상 `auth.seq` 본인만 조회)를 새로 추가하고, 소단원별 집계 로직은 `Dashboard_getClassSummary`와 `Dashboard_buildStudentSubunitSummary_`로 공유해 중복 제거 (→ 형성평가 정답률 막대그래프·성장 비교 그래프는 이후 제거, 교과서활동 점수 + 챗봇 평가만 남음)
- [x] **형성평가 앱 제거** — 형성평가를 종이 학습지로 진행하게 되면서, 앱에서 형성평가 관련 기능을 전부 뺌. `frontend/formative/`, `apps-script/Formative.js` 삭제, `submitFormativeResponse`/`getFormativeAttemptCounts` 라우팅 제거, `Sync_formativeQuestions`(Firestore `formativeQuestions` 동기화) 제거, "반별 오늘 활동 지정"의 활동 목록에서 형성평가 옵션 제거, 점수 시스템에서 "문제풀이 점수"(형성평가 정답 적립분) 제거(교과서활동 점수만 남음), 교사 대시보드·내 대시보드·학부모 포털에서 형성평가 정답률/성장 비교/응답 이력 표시 제거(진단평가·챗봇 참여·교과서활동 점수는 그대로 유지). `형성평가_문항` 시트는 삭제하지 않고 "관련 내용 답하기" 챗봇의 개념→성취기준 참조 표로만 계속 사용. `형성평가_응답` 시트도 과거 기록 보존을 위해 그대로 둠(더 이상 새 행은 안 쌓임)
