...
function homeUrl() {
  ...
  const subfolders = ['diagnostic', 'chatbot', 'roster', 'dashboard', 'parent', 'eval-review', 'activity', 'my-dashboard'];
  const inSubfolder = subfolders.some((f) => path.includes(`/${f}/`));
  return inSubfolder ? '../index.html' : 'index.html';
}

/**
 * 하위 폴더(diagnostic/, chatbot/, roster/, dashboard/, parent/, eval-review/,
 * activity/, my-dashboard/)의 페이지에서 호출합니다. 로그인되어 있지 않으면
 * 루트 로그인 화면으로 돌려보냅니다.
 */
